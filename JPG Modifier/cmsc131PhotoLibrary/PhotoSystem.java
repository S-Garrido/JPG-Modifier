...
public class PhotoSystem {
	...
	static final long serialVersionUID = 20191013;

	static final int NUM_RADIO_BUTTONS = 14;

	
	/**
	 * Begin the application.
	 */
	public static void begin(double displayScaleIn, String initialPhoto) {



		JRadioButton[] radioButtons = new JRadioButton[NUM_RADIO_BUTTONS];
		radioButtons[0] = new JRadioButton("Red Only");
		radioButtons[1] = new JRadioButton("Green Only"); 
		radioButtons[2] = new JRadioButton("Blue Only");
		
		radioButtons[3] = new JRadioButton("Grayscale");
		radioButtons[4] = new JRadioButton("Artistic");
		radioButtons[5] = new JRadioButton("Gradient");
		
		radioButtons[6] = new JRadioButton("Censor It");
		radioButtons[7] = new JRadioButton("Horizontal Stretch");
		radioButtons[8] = new JRadioButton("Vertical Stretch");
		
		radioButtons[9] = new JRadioButton("Mirror It");
		radioButtons[10] = new JRadioButton("Me and My Mirror");
		radioButtons[11] = new JRadioButton("Rotated");
		
		radioButtons[12] = new JRadioButton("Upside Down");
		radioButtons[13] = new JRadioButton("Wacky");


		for (int i=0; i<radioButtons.length; i++) {
	 		RescaleHelpers.rescaleGUI(radioButtons[i], displayScaleIn);
		}
			...
			try {
				...
				while(displaying) {
					synchronized(displayedPhoto) {
						...
						new PhotoFrame(displayScaleIn, flag, radioButtons, displayedPhoto, 
								fileToOpen, 0, 0);
						displayedPhoto.wait();
						switch (flag[0]) { 
						case 0: //modify
	...
	public static Photograph doModification(JRadioButton[] radioButtons, Photograph displayedPhoto) {
		if (radioButtons[0].isSelected()) {
			...
			return PhotoTools.isolateColor(displayedPhoto, 2);					
			...
			return PhotoTools.gradientFilter(displayedPhoto);
		...
		} else if (radioButtons[12].isSelected()) {
			return PhotoTools.upsideDown(displayedPhoto);
		} else if (radioButtons[13].isSelected()) {
